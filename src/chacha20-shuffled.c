...
// seed for PRNG
// rngstate should not be '0', use other values
uint64_t rngstate[2];

// modern PRNG https://xoshiro.di.unimi.it/xorshift128plus.c
uint64_t xorshift128plus(void) {
    uint64_t x = rngstate[0];
    uint64_t const y = rngstate[1];
    rngstate[0] = y;
    x ^= x << 23; // a
    rngstate[1] = x ^ y ^ (x >> 17) ^ (y >> 26); // b, c
    return rngstate[1] + y;
}
...
static void chacha20_block(uint32_t in[16], uint8_t out[64], int num_rounds) {
    ...
    for (i = num_rounds; i > 0; i -= 2) {
        /* ########################
         * How to shuffle the sequence randomly?
         * 1. Use an array with current sequence, shuffle this, execute it in the way of the new sequence
         *   => how to implement shuffling an array???
         * 2. Use a temp array which holds not executed rounds while get some random until all rounds are executed
         *   => better way here, use another variable for detecting finished sequence
         */ // ####################

        // column round
        /* ########################
         * following code implements shuffled:
        ...
        _Bool finish = 0;
        while (finish == 0){
            int rnd = xorshift128plus() % num_rounds;
            int random = rnd;
            for(int k = 0; k < rnd; k++){
                random = xorshift128plus() % 4;
            }
            if (random == 0 && done[random] == 0){
                chacha20_quarterround(x, 0, 4,  8, 12);
                done[random] = 1;
            }else if (random == 1 && done[random] == 0){
                chacha20_quarterround(x, 1, 5,  9, 13);
                done[random] = 1;
            }else if (random == 2 && done[random] == 0) {
                chacha20_quarterround(x, 2, 6, 10, 14);
                done[random] = 1;
            }else if (random == 3 && done[random] == 0) {
                chacha20_quarterround(x, 3, 7, 11, 15);
                done[random] = 1;
            }
        ...
        while (finish == 0) {
            ...
            if (random == 0 && done1[random] == 0) {
                chacha20_quarterround(x, 0, 5, 10, 15);
                done1[random] = 1;
            } else if (random == 1 && done1[random] == 0) {
                chacha20_quarterround(x, 1, 6, 11, 12);
                done1[random] = 1;
            } else if (random == 2 && done1[random] == 0) {
                chacha20_quarterround(x, 2, 7, 8, 13);
                done1[random] = 1;
            } else if (random == 3 && done1[random] == 0) {
                chacha20_quarterround(x, 3, 4, 9, 14);
                done1[random] = 1;
            }

            if (done1[0] == 1 && done1[1] == 1 && done1[2] == 1 && done1[3] == 1) {
                finish = 1;
            }
        }
        /* END */
...
static void chacha20_init_state(uint32_t s[16], uint8_t key[32], uint32_t counter, uint8_t nonce[12]) {
    ...
    /*
     * Initialize PRNG with following seed
     * s[13] and s[14] are values of the nonce
     * in real life applications this should be some other value
     * => here nonce is usually changing every execution
     */
    rngstate[0] = s[13];
    rngstate[1] = s[14];
}
